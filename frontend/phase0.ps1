# Agentic AI Platform - Phase 0 Automation Script
...
# Now update server.ts to apply auth middleware to protected routes.
# This part is tricky because we don't know the exact server.ts structure.
# We'll attempt to modify it by adding the middleware to each route group.
# We'll create a backup and then insert the auth middleware after public routes.
...
# Check if we already have import of authenticate
...
# Now apply middleware to each route group except auth.
# We'll replace app.use('/api/agents', agentRoutes) with app.use('/api/agents', authenticate, agentRoutes)
$patterns = @(
    '/api/agents',
    '/api/messages',
    '/api/documents',
