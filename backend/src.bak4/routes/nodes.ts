...
router.post('/register', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const node = await registerNode(req.user.id, name, endpoint);
    res.json(node);
  } catch (error: any) {
...
// Complete a task (node would call this)
router.post('/tasks/:taskId/complete', async (req, res) => {
  try {
    ...
    const task = await completeTask(req.params.taskId, result, reward);
    res.json(task);
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
