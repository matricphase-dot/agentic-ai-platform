# fix_duplicates_last_fixed.ps1 – Ultimate duplicate removal with exact line matching
$ErrorActionPreference = "Continue"
$schemaPath = "prisma/schema.prisma"
$backupPath = "prisma/schema.prisma.bak-last-fixed"
        ...
        if ($line -match '^\s+(\w+)') {
            ...
            if ($line -match '^\s+\w+\s+\S') {
                if ($fieldNames.ContainsKey($fieldName)) {
                    # Also check if the exact line is a duplicate (but we already have a field with same name)
